...
\section{Implementation Details: Aspect Extraction Engine}
The following Python snippet demonstrates the hybrid aspect extraction logic used in Sentilytics AI, leveraging SpaCy dependency parsing to identify product attributes and their associated descriptors.

\begin{lstlisting}[language=Python, caption=Core Aspect Extraction Logic]
...
\section{Model Optimization: Dynamic Quantization}
To achieve high-speed inference on CPU environments, the model undergoes dynamic 8-bit quantization after fine-tuning. This process significantly reduces the model footprint while maintaining over 99\% of the original accuracy.

\begin{lstlisting}[language=Python, caption=Post-Training Dynamic Quantization]
import torch
...
# Saving the optimized weight dictionary
torch.save(quantized_model.state_dict(), "quantized_model.bin")
\end{lstlisting}

\section{Extended Statistical Data}

\begin{table}[H]
\tbl{Detailed Performance Metrics for Emotion Classification}
...
Fear & 0.82 & 0.80 & 0.81 \\
Surprise & 0.88 & 0.85 & 0.86 \\
Excitement & 0.89 & 0.87 & 0.88 \\
\hline
\textbf{Macro Average} & \textbf{0.87} & \textbf{0.85} & \textbf{0.86} \\
\hline
\end{tabular}}
